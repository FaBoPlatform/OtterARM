...
def setup_camera(device_id, width=320, height=240, fps=30):
    ...
    print(f"カメラ {device_id} の実際の解像度: {actual_width}x{actual_height} @ {actual_fps} FPS")
...
def save_data(num_pairs, camera, total_state_dim, dataset_dir, episode_len, episode_name, data_storage_list, camera_device_ids, camera_names, width, height, RECORD):
    """
    データを保存する関数。
    ...
    with h5py.File(dataset_path + '.hdf5', 'w', rdcc_nbytes=1024**2*2) as root:
        ...
        for device_id, cam_name in zip(camera_device_ids, camera_names):
            ...
            if not ret:
                ...
                print(f"可能な原因: カメラが接続されていない、使用中である、または解像度がサポートされていない可能性があります。")
        ...
        with tqdm(total=episode_len, desc="進行状況", unit="フレーム") as pbar:
            try:
                while not terminate_event.is_set():
                    ...
                    # 各カメラからフレームを取得
                    for device_id, cam_name in zip(camera_device_ids, camera_names):
                        ret[device_id], frame[device_id] = camera[device_id].read()
                        if not ret[device_id]:
                            print(f"カメラ {cam_name} (デバイスID: {device_id}) からフレームの取得に失敗しました。")
                            # 取得失敗時は黒い画像を用意
                            frame[device_id] = np.zeros((height, width, 3), dtype=np.uint8)
                            image_frames[device_id] = cv2.cvtColor(frame[device_id], cv2.COLOR_BGR2RGB)
                        else:
                            image_frames[device_id] = cv2.cvtColor(frame[device_id], cv2.COLOR_BGR2RGB)
    ...
    try:
        for episode in range(num_episodes):
            ...
            data_thread = threading.Thread(
                target=save_data,
                args=(num_pairs, cameras, total_state_dim, dataset_dir, episode_len, episode_name, data_storage_list, valid_camera_device_ids, valid_camera_names, width, height, RECORD)
            )
